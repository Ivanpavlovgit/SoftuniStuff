...
public class ControllerImpl implements Controller {
    ...
    public String addPeripheral (int computerId,int id,String peripheralType,String manufacturer,String model,double price,double overallPerformance,String connectionType) {
        ...
        if (peripherals.containsKey (id)) {
            throw new IllegalArgumentException (String.format (EXISTING_PERIPHERAL,peripheralType,id,computerId));
        }
    ...
    public String BuyBestComputer (double budget) {
        List<Computer> filteredByPriceAccordingToBudget = computers.values ()
                .stream ()
                .filter (c -> c.getPrice () <= budget)
                .sorted (Comparator.comparing (Computer::getOverallPerformance).reversed ())
                .collect (Collectors.toList ());
        if (filteredByPriceAccordingToBudget.isEmpty ()) {
            throw new IllegalArgumentException (String.format (CAN_NOT_BUY_COMPUTER,budget));
        }
        Computer computer = filteredByPriceAccordingToBudget.get (0);
